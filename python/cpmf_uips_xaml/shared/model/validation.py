...
class ValidationError(Exception):
    ...
    def __init__(
        self, message: str, field_path: str = "", schema_violations: list[str] | None = None
    ) -> None:
        """Initialize validation error with details.

        Args:
            message: Error message
            field_path: Path to field that failed validation
            schema_violations: List of schema violation messages
        """
...
class OutputValidator:
    ...
    def validate_workflow_content(self, content: WorkflowContent) -> list[str]:
        ...
        if not isinstance(content.total_activities, int) or content.total_activities < 0:
            ...
            errors.append(
                f"total_activities ({content.total_activities}) != "
                f"len(activities) ({len(content.activities)})"
            )
        ...
        if not isinstance(content.total_arguments, int) or content.total_arguments < 0:
            ...
            errors.append(
                f"total_arguments ({content.total_arguments}) != "
                f"len(arguments) ({len(content.arguments)})"
            )
        ...
        if not isinstance(content.total_variables, int) or content.total_variables < 0:
            ...
            errors.append(
                f"total_variables ({content.total_variables}) != "
                f"len(variables) ({len(content.variables)})"
            )
    ...
    def validate_diagnostics(self, diagnostics: ParseDiagnostics) -> list[str]:
        ...
        if not isinstance(diagnostics.processing_steps, list):
            ...
            isinstance(step, str) and len(step.strip()) > 0 for step in diagnostics.processing_steps
        ):
    ...
    def _validate_variable(self, var: Any) -> list[str]:
        ...
        if (
            not hasattr(var, "scope")
            or not isinstance(var.scope, str)
            or len(var.scope.strip()) == 0
        ):
    ...
    def _validate_activity(self, activity: Any, activity_ids: set[str]) -> list[str]:
        ...
        if (
            not hasattr(activity, "tag")
            or not isinstance(activity.tag, str)
            or len(activity.tag.strip()) == 0
        ):
        ...
        if (
            not hasattr(activity, "depth_level")
            or not isinstance(activity.depth_level, int)
            or activity.depth_level < 0
        ):
...
def validate_output(result: ParseResult, strict: bool = True) -> list[str]:
    ...
    if strict and errors:
        raise ValidationError(
            f"Output validation failed with {len(errors)} errors", schema_violations=errors
        )
