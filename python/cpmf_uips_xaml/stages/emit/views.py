...
from ..assemble.index import ProjectIndex
from ..assemble.analyzer import ProjectAnalyzer
from ...shared.model.dto import ActivityDto
from ..normalize.provenance import create_provenance
...
class View(Protocol):
    ...
    All views must implement render(analyzer, index) -> dict.
    """

    def render(self, analyzer: ProjectAnalyzer, index: ProjectIndex) -> dict[str, Any]:
...
class NestedView:
    ...
    def render(self, analyzer: ProjectAnalyzer, index: ProjectIndex) -> dict[str, Any]:
        ...
        starting_workflows = self._find_starting_workflows(analyzer, index)
        ...
        for wf_id in starting_workflows:
            wf_dto = analyzer.get_workflow(wf_id)
            if not wf_dto or wf_id in visited:
                continue

            visited.add(wf_id)
            nested_wf = self._build_nested_workflow(wf_dto, analyzer, index, visited, depth=0)
    ...
    def _find_starting_workflows(self, analyzer: ProjectAnalyzer, index: ProjectIndex) -> list[str]:
        ...
        if index.entry_point_ids:
            return index.entry_point_ids

        # Find workflows with no incoming edges in call graph
        roots = []
        for wf_id in analyzer.workflows_graph.nodes():
            if not list(analyzer.call_graph.predecessors(wf_id)):
                roots.append(wf_id)
    ...
    def _build_nested_workflow(
        ...
        nested_activities = self._build_nested_activities(workflow_dto, analyzer, index, visited, depth)
    ...
    def _build_nested_activities(
        ...
        if depth < self.max_depth:
            for activity in workflow_dto.activities:
                ...
                # Get callee workflow
                callee_dto = analyzer.get_workflow(callee_wf_id)
                if not callee_dto:
                    continue
                ...
                nested_callee_wf = self._build_nested_workflow(
                    callee_dto, analyzer, index, new_visited, depth + 1
                )
...
class ExecutionView:
    ...
    def render(self, analyzer: ProjectAnalyzer, index: ProjectIndex) -> dict[str, Any]:
        ...
        # Resolve entry point
        entry_wf_id = self._resolve_entry_point(analyzer, index, self.entry_point)
        if not entry_wf_id:
        ...
        for wf_id, wf_dto, depth in analyzer.call_graph.traverse_dfs(
            ...
            nested_activities = self._build_nested_activities(
                wf_dto, analyzer, index, visited_workflows, depth
            )
    ...
    def _resolve_entry_point(self, analyzer: ProjectAnalyzer, index: ProjectIndex, entry: str) -> str | None:
        """Resolve entry point to workflow ID."""
        if analyzer.workflows_graph.has_node(entry):
            return entry
...
class SliceView:
    ...
    def render(self, analyzer: ProjectAnalyzer, index: ProjectIndex) -> dict[str, Any]:
        ...
        context = analyzer.slice_context(self.focus, index, self.radius)

        if not context:
            return {"error": f"Activity not found: {self.focus}"}

        # Get focal activity
        focal = analyzer.get_activity(self.focus)
        if not focal:
            return {"error": f"Activity not found: {self.focus}"}

        # Build parent chain
        parent_chain = self._build_parent_chain(self.focus, analyzer, index)

        # Get siblings
        siblings = self._get_siblings(self.focus, analyzer, index)

        # Get workflow
        workflow = analyzer.get_workflow_for_activity(self.focus, index)
    ...
    def _build_parent_chain(self, activity_id: str, analyzer: ProjectAnalyzer, index: ProjectIndex) -> list[ActivityDto]:
        """Build parent chain from root to focal activity."""
        chain = []
        current_id = activity_id

        while True:
            preds = analyzer.activities_graph.predecessors(current_id)
            if not preds:
                break

            parent_id = preds[0]
            parent = analyzer.get_activity(parent_id)
            if not parent:
                break
    ...
    def _get_siblings(self, activity_id: str, analyzer: ProjectAnalyzer, index: ProjectIndex) -> list[ActivityDto]:
        """Get sibling activities (same parent)."""
        siblings: list[ActivityDto] = []

        preds = analyzer.activities_graph.predecessors(activity_id)
        if not preds:
            return siblings

        parent_id = preds[0]

        for child_id in analyzer.activities_graph.successors(parent_id):
            if child_id != activity_id:
                child = analyzer.get_activity(child_id)
                if child:
