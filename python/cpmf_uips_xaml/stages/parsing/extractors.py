...
NOTE: ActivityUtils import is a remaining boundary violation that should be
fixed by passing an expression parser interface to ActivityExtractor.
"""
...
from ...shared.model.models import Activity, Expression, WorkflowArgument, WorkflowVariable
from ...shared.utils.annotations import parse_annotation
...
class ArgumentExtractor:
    ...
    def __init__(self, platform: "XamlDialect") -> None:
        """Initialize with platform configuration.

        Args:
            platform: Platform-specific constants for argument parsing
        """
        self.platform = platform

    def extract_arguments(self, root: ET.Element, namespaces: dict[str, str]) -> list[WorkflowArgument]:
        ...
        for prop in members.findall(f"{{{x_ns}}}Property"):
            argument = self._extract_single_argument(prop, sap2010_ns)
            if argument:
                arguments.append(argument)

        return arguments

    def _extract_single_argument(self, prop: ET.Element, sap2010_ns: str) -> WorkflowArgument | None:
...
class VariableExtractor:
    ...
    def __init__(self, platform: "XamlDialect") -> None:
        ...
        Args:
            platform: Platform-specific constants for variable parsing
        """
        self.platform = platform

    def extract_variables(self, root: ET.Element, namespaces: dict[str, str]) -> list[WorkflowVariable]:
        """Extract all variables from workflow with scope information."""
        variables = []

        # Find all Variable elements throughout the tree
        for elem in root.iter():
            if self._is_variable_element(elem):
                variable = self._extract_single_variable(elem)
                if variable:
                    variables.append(variable)

        return variables

    def _is_variable_element(self, elem: ET.Element) -> bool:
        """Check if element represents a variable definition."""
        tag = elem.tag.split("}")[-1] if "}" in elem.tag else elem.tag
        return tag == "Variable" or tag.endswith("Variable") or "Variable" in tag

    def _extract_single_variable(self, elem: ET.Element) -> WorkflowVariable | None:
        ...
        scope = self._determine_scope(elem)

        return WorkflowVariable(name=name, type=type_attr, default_value=default_value, scope=scope)

    def _determine_scope(self, elem: ET.Element) -> str:
...
class ActivityExtractor:
    ...
    def __init__(self, platform: "XamlDialect", config: dict[str, Any]) -> None:
        """Initialize with platform and parser configuration.

        Args:
            platform: Platform-specific constants for activity parsing
            config: Parser configuration
        """
        self.platform = platform
        self.activity_utils = platform.activity_utils
        self.config = config
        self._activity_counter = 0
        self._activity_cache: dict[
            tuple[int, str, str, str | None, int, str], Activity | None
        ] = {}  # Cache for repeated element processing
    ...
    def extract_activities(
        ...
        def process_element(elem: ET.Element, parent_id: str | None = None, depth: int = 0) -> None:
            ...
            if self._is_activity(elem, tag_name):
                activity_data = self._extract_single_activity(
                    elem, tag_name, namespaces, parent_id, depth
                )
    ...
    def _is_activity(self, elem: ET.Element, tag_name: str) -> bool:
        ...
        if tag_name in self.platform.core_visual_activities:
            return True
        ...
        # Check namespace (platform activities)
    ...
    def _extract_single_activity(
        ...
        # Extract annotation
        annotation, annotation_block = self._extract_annotation(elem, namespaces.get("sap2010", ""))
    ...
    def _extract_annotation(self, elem: ET.Element, sap2010_ns: str) -> tuple[str | None, Any]:
        """Extract annotation text and structured block from activity.

        Returns:
            Tuple of (raw_text, parsed_block)
        """
        if not sap2010_ns:
            return None, None
    ...
    def _extract_activity_variables(
        self, elem: ET.Element, activity_id: str
    ...
    def _extract_expressions(self, elem: ET.Element) -> list[Expression]:
        ...
        for key, value in elem.attrib.items():
            if self._is_expression(value):
                if use_parser:
                    ...
                    parsed = self.activity_utils.parse_expression(value, language)
        ...
        if elem.text and self._is_expression(elem.text):
            ...
            if use_parser:
                # Use new tokenizer-based parser
                parsed = self.activity_utils.parse_expression(text_content, language)
                expr = parsed.to_expression(
    ...
    def extract_activity_instances(
        self, root: ET.Element, namespaces: dict[str, str], workflow_id: str, project_id: str
    ) -> list[Activity]:
        ...
        _ = get_visible_elements(root, self.platform.platform_namespace)  # Used for validation
        ...
        def process_element(
            ...
            if not is_visible_element(elem, self.platform.platform_namespace):
            ...
            activity = self._extract_single_activity_instance(
                elem, namespaces, workflow_id, project_id, parent_activity_id, depth, node_path
            )
    ...
    def _extract_single_activity_instance(
        self,
        element: ET.Element,
        namespaces: dict[str, str],
        workflow_id: str,
        ...
        selectors = self.activity_utils.extract_selectors_from_config(configuration)

        # Extract annotation
        annotation, annotation_block = self._extract_annotation(element, namespaces.get("sap2010", ""))
        ...
        activity_id = self.activity_utils.generate_activity_id(
            project_id, workflow_id, node_id, activity_content
        )

        # Determine visibility and container type
        is_visible = element in get_visible_elements(
            element.getroot() if hasattr(element, "getroot") else element
        )
    ...
    def _extract_activity_arguments(self, element: ET.Element) -> dict[str, Any]:
        ...
        for attr_name, attr_value in element.attrib.items():
            ...
            if not (
                attr_name.startswith("xmlns")
                or "ViewState" in attr_name
                or "HintSize" in attr_name
                or "IdRef" in attr_name
            ):
    ...
    def _extract_activity_metadata(self, element: ET.Element) -> dict[str, Any]:
        ...
        metadata_attrs = ["ViewState", "HintSize", "IdRef", "VirtualizedContainerService"]
    ...
    def _extract_business_logic_expressions(self, element: ET.Element) -> list[str]:
        ...
        for attr_value in element.attrib.values():
            extracted_expressions = self.activity_utils.extract_expressions_from_text(attr_value)
    ...
    def _extract_variable_references(
        self, element: ET.Element, expressions: list[str], arguments: dict[str, Any]
    ) -> list[str]:
        ...
        for expr in expressions:
            vars_in_expr = self.activity_utils.extract_variable_references(expr)
            variables.extend(vars_in_expr)

        # Extract from argument values
        for arg_value in arguments.values():
            if isinstance(arg_value, str):
                vars_in_arg = self.activity_utils.extract_variable_references(arg_value)
                variables.extend(vars_in_arg)

        return list(set(variables))  # Remove duplicates

    def _serialize_activity_for_hashing(
        self,
        activity_type: str,
