...
def is_visible_element(element: ET.Element, platform_namespace: str = "") -> bool:
    """Determine if XML element represents a visually shown activity.

    Args:
        element: XML element from XAML tree
        platform_namespace: Platform-specific namespace for activity detection (optional)
    ...
    # Platform-specific namespace check (if provided)
    if platform_namespace and element.tag.startswith(f"{{{platform_namespace}}}"):
        return True

    return False


def get_visible_elements(root: ET.Element, platform_namespace: str = "") -> list[ET.Element]:
    ...
    def traverse(elem: ET.Element) -> None:
        if is_visible_element(elem, platform_namespace):
